...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'mark_all') {
    ...
    header('Location: /tracky/staff/staff_notifications.php'); exit;
}
...
<div class="page-header">
  <div class="page-header-left">
    <h4>Notifikasi</h4>
    <p>Kemaskini pesanan, tugasan & pengumuman penting</p>
  </div>
...
document.querySelectorAll('.btn-mark').forEach(btn => btn.addEventListener('click', async () => {
  await apiPost('/tracky/api/staff_mark_notification.php', { notification_id: parseInt(btn.dataset.id) });
  location.reload();
}));
